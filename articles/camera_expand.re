...
== 18. USB MSC 機能を使う
拡張ボードのSDカードをホストPCから直接参照することが可能になります。
...
こちらはUART等のシリアルコンソールへログを出力する機能です。

【26.2. RAM log 機能】

こちらRAMバッファへ出力するログ機能です。dmesgコマンドによってまとめてコンソールに出力できます。
...
charset以外は写真だとわかりずらいですが動きがあるアプリケーションなのでチュートリアルに記載の環境がある方は
確認してみると面白いと思います。
...
=== USB MSC
@<kw>{USB MSC, Mass Storage Class}を有効にするとホストPCから拡張ボード上のSDカードへ直接アクセスすることができます。

動作確認方法を次に書きます。
拡張ボードにSDカードが挿入されている状態で、拡張ボードのUSB端子とホストPCをUSBケーブルで接続します。
...
@<img>{usbmsc}のようにホストPCに拡張ボードのSDカードがリムーバブルディスクとして認識されます。
...
USB MSC機能があるおかげで画像ファイルの確認でマイクロSDを取り外しする必要がなくなります。
動作確認に非常に役立つ機能です。
...
トレースデータはtrace.logといるファイル名で保存しました。
次が実行したコマンドです。
